...
class SocketBuffer(object):
    def __init__(self, socket, socket_read_size):
        ...
        self._buffer = io.BytesIO()   # 将socket里面的内容读取放到buffer里面
    ...
    @property
    def length(self):
        return self.bytes_written - self.bytes_read   # 缓冲区中有效数据长度

    def _read_from_socket(self, length=None):
        # 从socket读取到buffer， 对于buffer来说是写入
        socket_read_size = self.socket_read_size
        buf = self._buffer
        buf.seek(self.bytes_written)   # self.bytes_written是已经写入到buffer里面的长度
        marker = 0
    ...
    def read(self, length):
        # 从buffer中取length长度的内容并去除最后的CRLF符， 对buffer来说是读出
        length = length + 2  # make sure to read the \r\n terminator
        # make sure we've read enough data from the socket
        if length > self.length:   # 如果buffer里面的长度不够，说明还要继续读一部分
    ...
    def readline(self):
        ...
        while not data.endswith(SYM_CRLF):
            ...
            buf.seek(self.bytes_read)  # BytesIO的数据一直在那，不是读走了就没了， 所以self._read_from_socket()之后要重新seek回到原来的位置接着读
            data = buf.readline()
...
class PythonParser(BaseParser):
    ...
    def read_response(self):
        ...
        # bulk response, 只读取头部，后面的还接着读
        elif byte == '$':
            length = int(response)
            if length == -1:
                return None
            response = self._buffer.read(length)
        # multi-bulk response 数组是嵌套类型，也要接着读
        elif byte == '*':
            length = int(response)
            if length == -1:
                return None
...
class Connection(object):
    ...
    def pack_command(self, *args):
        ...
        command = args[0]   # SET或CONFIG SET 等
        if ' ' in command:    # 双词命令
            args = tuple(Token.get_token(s)  # get_token返回不编码的字符串
                         for s in command.split()) + args[1:]
        else:    # 单个词命令，命令后面的参数不包括
            args = (Token.get_token(command),) + args[1:]

        buff = SYM_EMPTY.join((SYM_STAR, str(len(args)).encode(), SYM_CRLF))   # 计算命令行总长度
...
class ConnectionPool(object):
    ...
    def _checkpid(self):
        if self.pid != os.getpid():
            with self._check_lock:    # 考虑新进程下多线程操作的情况
                if self.pid == os.getpid():
                    # another thread already did the work while we waited
                    # on the lock.
                    return
                self.reset()   # 如果是新进程，就重新初始化连接池而不是共用
