...
fat = 10000
step_num = [1*fat, 10*fat, 100*fat, 500*fat, 1000*fat, 2000*fat, 3000*fat, 4000*fat, ]  # 5000*fat, 8000*fat, 10000*fat
lable = ['car', 'money', 'food', 'book', 'movie', 'cake', 'mall']

mongo_obj = MongoDB()
table_name = 'day_result_4m'

# db.day_result.createIndex({"datestr":1,"age":1})
...
def insert_test(start, end, batch_size=100):
    ...
    for i in range(start, end):
        ...
        data = {
            ...
            'profile_id': str(uuid.uuid1()),   # 如果不转字符串会报UUID object has no attribute ‘translate’
        ...
        if count % batch_size == 0:
            s_time = time.time()
            ret = mongo_obj.insert_many(tmp, table_name)
            if ret == False:
                print('insert failed')
                sys.exit(1)
            e_time = time.time()
...
def query_test():
    ...
    for item in range(num):
        filter1 = {
            ...
            'gender': random.choice(['male', 'female']),
            'age': random.randint(0,90),  # {'age': {'$gt': 20}}
        }
        s_time = time.time()
        data = mongo_obj.find_many(table_name, filter1)
        if data == False:
            print('query failed')
            sys.exit(1)
